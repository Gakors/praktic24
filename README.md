...
$$ 
f(x) = (x - pi/3)**2*(-10*exp(-4*pi/3) + 9*sqrt(3)*exp(-4*pi/3)/4) + (x - pi/3)*(5*exp(-4*pi/3)/2 + 2*sqrt(3)*exp(-4*pi/3)) - sqrt(3)*exp(-4*pi/3)/2
$$
...
````python
x = sym.symbols('x')
gx = (x**2-5*x+7)/(x-3) # функция
print(gx)
````

$$
g(x) = \frac{x^2-5*x+7}{x-3}
$$
